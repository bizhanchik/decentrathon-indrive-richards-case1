#!/usr/bin/env python3
"""
YOLOv8s training script for car defect detection.
Supports GPU/CPU training, checkpointing, and resume capability.
Enables AMP (mixed precision) training when CUDA is available.
"""
...
class YOLOTrainer:
    def __init__(self, data_yaml_path: str, model_size: str = 's', 
                 epochs: int = 10, batch_size: int = 16, 
                 checkpoint_interval: int = 5, project_name: str = 'car_defect_detection_small'):
        """
        ...
        self.output_dir = self.runs_dir / 'train' / f'yolov8{model_size}'
        self.checkpoints_dir = self.output_dir / 'weights'
    ...
    def _get_latest_checkpoint(self) -> str:
        ...
        # Check for last.pt first (most recent checkpoint)
        last_pt = self.checkpoints_dir / 'last.pt'
        if last_pt.exists():
            return str(last_pt)
            
        # Otherwise look for any checkpoint
    ...
    def _find_best_checkpoint(self) -> str:
        """Find the best checkpoint from previous runs."""
        # Look in runs directory for car_defect_detection* folders
        detection_dirs = list(self.runs_dir.glob("car_defect_detection*"))
        ...
        # Check for best.pt in the weights directory
        best_pt = latest_dir / 'weights' / 'best.pt'
        if best_pt.exists():
            print(f"Found best checkpoint from previous run: {best_pt}")
            return str(best_pt)
            
        # If no best.pt, check for last.pt
        last_pt = latest_dir / 'weights' / 'last.pt'
        if last_pt.exists():
            print(f"No best checkpoint found, using last checkpoint: {last_pt}")
            return str(last_pt)
    ...
    def train(self, resume: bool = False, from_best: bool = False, pretrained_weights: str = None, workers: int = 8) -> str:
        """
        Train the YOLOv8 model.
        
        Args:
            resume: Whether to resume from the latest checkpoint
            from_best: Whether to start training from the best checkpoint of previous runs
        ...
        if resume:
            ...
            if best_checkpoint:
                print(f"Starting training from best checkpoint: {best_checkpoint}")
                model = YOLO(best_checkpoint)
            else:
                print("No best checkpoint found, starting fresh training")
                model = YOLO(f'yolov8{self.model_size}.pt')
        ...
        training_config = {
            ...
            'resume': resume,  # Enable resume functionality
        }
        ...
        try:
            ...
            latest_checkpoint = self._get_latest_checkpoint()
            if latest_checkpoint:
                print(f"You can resume training with: python yolov8s/train_small.py --resume")
            raise
...
def main():
    parser = argparse.ArgumentParser(description='Train YOLOv8s for car defect detection')
    ...
    parser.add_argument('--from-best', action='store_true',
                       help='Start training from the best checkpoint of previous runs')
    parser.add_argument('--pretrained', type=str, default=None,
                       help='Path to custom pretrained weights')
    parser.add_argument('--project', type=str, default='train/yolov8s',
    ...
    trainer = YOLOTrainer(
        ...
        model_size='s',  # Fixed to 's' for YOLOv8s
        epochs=args.epochs,
